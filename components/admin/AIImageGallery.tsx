...
function SortableAIImage({ id, image }: { id: string; image: string }) {
...
export function AIImageGallery({ images: initialImages }: AIImageGalleryProps) {
  ...
  const handleDelete = async (image: string) => {
    if (!confirm('Delete this AI image? This cannot be undone.')) return;
    ...
    <div>
      ...
      <div className="mb-6 flex items-center justify-between bg-white/5 border border-white/10 rounded-lg p-4">
        <div className="text-sm text-white/50">
          {isReorderMode ? 'Drag images to reorder them' : `${images.length} AI images`}
        </div>
        ...
        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
          <SortableContext items={images} strategy={rectSortingStrategy}>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
              {images.map((image) => (
                <SortableAIImage key={image} id={image} image={image} />
              ))}
            </div>
          </SortableContext>
        </DndContext>
      ...
      {images.length === 0 && (
        <div className="text-center py-24 border border-white/10 rounded-lg">
          <p className="text-white/50 mb-4">No AI images found</p>
          <p className="text-sm text-white/30">Add images to public/ai-images and seed from files</p>
        </div>
      )}
    </div>
  );
