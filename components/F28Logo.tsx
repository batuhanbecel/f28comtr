...
/**
 * Animated f/2.8 logo — stroke draw-on followed by fill fade-in.
...
export function F28Logo({
  ...
  const replay = useCallback(() => {
    ...
    const els = svgRef.current.querySelectorAll<SVGElement>('.f28-path, .f28-fill');
    els.forEach((el) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <g
          className="f28-fill"
          style={{ animationDelay: `calc(${d} + 1.6s)` }}
        >
        ...
        {/* Stroke draw layer */}
        <path className="f28-path p1" pathLength={1} style={{ animationDelay: d }}
        ...
        />
        <path className="f28-path" pathLength={1} style={{ animationDelay: `calc(${d} + 0.3s)` }}
        ...
        />
        <polygon className="f28-path" pathLength={1} style={{ animationDelay: `calc(${d} + 0.6s)` }}
          points="246.84 641.85 207.98 641.85 482.83 275.72 521.9 275.97 246.84 641.85"
        />
        <path className="f28-path" pathLength={1} style={{ animationDelay: `calc(${d} + 0.5s)` }}
          d="M713.35,641.85h-336.61s292.88-391.03,292.88-391.03c33.71-44.92,36.78-105.79,7.44-153.65-28.25-46.08-81.77-72.17-136.41-64.21-67.51,9.84-117.59,68.04-117.43,135.69l-31.3-.04c-.2-88.96,69.84-163.12,158.9-168.06l19.3.02c58.62,3.24,110.57,37.81,137.4,86.65,30.41,55.36,27.2,120.65-5.66,172.17l-6.15,8.76-256.75,342.24h274.04s.33,31.46.33,31.46Z"
        />
        <polygon className="f28-path" pathLength={1} style={{ animationDelay: `calc(${d} + 1s)` }}
