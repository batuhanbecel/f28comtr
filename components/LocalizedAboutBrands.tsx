...
export function LocalizedAboutBrands({ partnerLogos, clientLogos }: LocalizedAboutBrandsProps) {
  ...
  return (
    <>
      {/* Partner Agencies */}
      <section className="py-20 md:py-28 px-8 md:px-16 border-t border-th-fg/[0.07]">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-6 mb-12">
            ...
            <div className="flex-1 h-px bg-th-fg/[0.06] hidden md:block" />
            <span className="text-th-fg/15 font-mono text-[10px] tracking-[0.3em] hidden md:block">
          ...
          <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-8 gap-[1px] bg-th-fg/[0.04] border border-th-fg/[0.06] overflow-hidden">
            {partnerLogos.map((logo) => {
              ...
              return (
                <div key={logo} className="relative aspect-square flex items-center justify-center bg-th-bg group hover:bg-th-fg/[0.03] transition-colors duration-500">
                  <Image
                    src={logo} alt={name} fill
                    className="object-contain brightness-0 dark:invert opacity-25 group-hover:opacity-70 transition-all duration-500 p-5 md:p-6 group-hover:scale-105"
            ...
            {clientLogos.map((logo) => {
              ...
              return (
                <div key={logo} className="relative aspect-square flex items-center justify-center bg-th-bg group hover:bg-th-fg/[0.03] transition-colors duration-500">
                  <Image
                    src={logo} alt={name} fill
                    className="object-contain brightness-0 dark:invert opacity-20 group-hover:opacity-65 transition-all duration-500 p-4 md:p-5 group-hover:scale-105"
